...
$lines = @(Get-Content temp/rebuild-phase2-complete.txt)
...
if ($sorted.Count -eq 0) {
    Write-Host "✅ NO xUnit warnings remaining!" -ForegroundColor Green
} else {
...
# Summary
$totalXunit = if ($xunitWarnings.Values) { $xunitWarnings.Values | ForEach-Object { $_.Values | Measure-Object -Sum | Select-Object -ExpandProperty Sum } | Measure-Object -Sum | Select-Object -ExpandProperty Sum } else { 0 }
