...
public class LearningRetrievalService : ILearningRetrievalService
{
    ...
    private readonly LearningStorageService _storageService;
    ...
    public LearningRetrievalService(
        ...
        LearningStorageService storageService,
        ...
        CancellationToken ct = default)
    {
        ...
        _logger.LogInformation(
            "Retrieving learnings for task: {TaskGoal} (MinConfidence: {MinConfidence}, MinSimilarity: {MinSimilarity}, MaxResults: {MaxResults})",
            context.TaskGoal, context.MinConfidence, context.MinSimilarity, context.MaxResults);

        // Get all learnings with embeddings
        var allLearnings = await _storageService.GetEmbeddedLearningsAsync(ct).ConfigureAwait(false);
        ...
        _logger.LogDebug(
            "After filtering: {Count} active learnings meet criteria",
        ...
        try
        {
            queryEmbedding = await _embeddingGenerator.GenerateEmbeddingAsync(queryText, ct).ConfigureAwait(false);
        ...
        _ = Task.Run(async () =>
        {
            foreach (var retrieved in topLearnings)
            {
                try
                {
                    ...
                    await _storageService.UpdateLearningAsync(retrieved.Learning, ct).ConfigureAwait(false);
                }
                catch (Exception ex)
                {
